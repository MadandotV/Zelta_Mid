...
class EnhancedMFI_RSI_MACD_Stochastic_Strategy:
    ...
    def calculate_mfi(self, period=10):
    ...
    def calculate_stochastic(self, period=10):
    ...
    def detect_divergence(self, price, indicator, window=10):
    ...
    def generate_signals(self):
        ...
        drop_factor = 0.001800

        current_signal = 0
        for i in range(1, len(self.data)):
            # Check for buy signal
            if ((self.data['mfi'].iloc[i-1] < 35) and (self.data['rsi'].iloc[i-1] < 35) and 
                macd_crossover_up.iloc[i-1] and (self.data['stoch_k'].iloc[i-1] < 25) and 
                (rsi_bull_div.iloc[i-1] or macd_bull_div.iloc[i-1])) or \
               ((self.data['mfi'].iloc[i-1] < 45) and (self.data['rsi'].iloc[i-1] < 45) and 
                macd_crossover_up.iloc[i-1] and stoch_crossover_up.iloc[i-1]):
                current_signal = 1  # Set buy signal

            # Check for sell signal
            elif ((self.data['mfi'].iloc[i-1] > 65) and (self.data['rsi'].iloc[i-1] > 65) and 
                  macd_crossover_down.iloc[i-1] and (self.data['stoch_k'].iloc[i-1] > 75) and 
                  (rsi_bear_div.iloc[i-1] or macd_bear_div.iloc[i-1])) or \
                 ((self.data['mfi'].iloc[i-1] > 55) and (self.data['rsi'].iloc[i-1] > 55) and 
                  macd_crossover_down.iloc[i-1] and stoch_crossover_down.iloc[i-1]):
                current_signal = -1  # Set sell signal
            elif current_signal == 1 and (self.data['mfi'].iloc[i-1]>65 or self.data['rsi'].iloc[i-1]>65 ):
                current_signal = 0  # Reset signal on potential dip
            # Check for potential rise (when in a sell position)
            elif current_signal == -1 and (self.data['mfi'].iloc[i-1]<35 or self.data['rsi'].iloc[i-1]<35):
